...
export default class PanelsMjml {
  ...
  restylePanels() {
    ...
    if (typeof this.panelManager.getButton('views', 'fullscreen') !== 'undefined') {
      this.panelManager.getButton('views', 'fullscreen').set({
        className: '',
        // resets the custom classes, so only displays the default ones and we don't render the icon font anymore
        label: `<svg style="display: block; max-width: 22px" viewBox="0 0 24 24">
    ...
    if (typeof this.panelManager.getButton('views', 'sw-visibility') !== 'undefined') {
      this.panelManager.getButton('views', 'sw-visibility').set({
        className: '',
        // resets the custom classes, so only displays the default ones and we don't render the icon font anymore
        label: `<svg style="display: block; max-width: 22px" viewBox="0 0 24 24">
